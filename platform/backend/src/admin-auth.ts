...
if (process.env.NODE_ENV === 'production' && (!process.env.COOKIE_SECRET || process.env.COOKIE_SECRET.length < 32)) throw new Error('COOKIE_SECRET must be at least 32 characters in production');
...
export async function login(email: string, password: string, reply: FastifyReply) {
  ...
  reply.setCookie(COOKIE_NAME, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', path: '/api/v1/admin', expires: expiresAt });
  return true;
}
