...
app.post('/api/v1/admin/logout', async (request, reply) => { await logout(request, reply); return { ok: true }; });
app.get('/api/v1/admin/me', async (request, reply) => { const user = await requireAdmin(request, reply); return user ? { user } : undefined; });

app.post('/api/v1/admin/imports', async (request, reply) => {
  const user = await requireAdmin(request, reply, [...mutateRoles]); if (!user) return;
  const part = await request.file(); if (!part) return reply.code(400).send({ error: 'FILE_REQUIRED' });
  const buffer = await part.toBuffer(); let records;
  ...
  if (!records.length) return reply.code(400).send({ error: 'EMPTY_FILE' });
  if (records.length > 100000) return reply.code(400).send({ error: 'TOO_MANY_ROWS', maxRows: 100000 });
  const batch = await prisma.importBatch.create({ data: { filename: part.filename, fileType: part.mimetype || 'application/octet-stream', uploadedById: user.id, totalRows: records.length, metadata: { columns: Object.keys(records[0] || {}) } } });
  await prisma.importRow.createMany({ data: records.map((payload, index) => ({ batchId: batch.id, rowNumber: index + 2, payload: payload as any })) });
...
app.get('/api/v1/admin/imports', async (request, reply) => {
  ...
  return { data: await prisma.importBatch.findMany({ orderBy: { uploadedAt: 'desc' }, take: 100, select: { id: true, filename: true, status: true, totalRows: true, validRows: true, errorRows: true, uploadedAt: true, validatedAt: true, approvedAt: true, publishedAt: true } }) };
});
...
app.post('/api/v1/admin/imports/:id/review', async (request, reply) => {
  const user = await requireAdmin(request, reply, [...reviewRoles]); if (!user) return;
  const { id } = request.params as { id: string }; const body = request.body as { decision?: 'APPROVE' | 'REJECT'; comment?: string };
  ...
  await prisma.$transaction([
    prisma.importReview.create({ data: { batchId: id, reviewerId: user.id, decision: body.decision, comment: body.comment } }),
    prisma.importBatch.update({ where: { id }, data: { status, ...(status === 'APPROVED' ? { approvedAt: new Date() } : {}) } })
  ]);
  await audit(user.id, `IMPORT_${status}`, 'ImportBatch', id, undefined, { comment: body.comment });
  return { id, status };
});

app.post('/api/v1/admin/imports/:id/publish', async (request, reply) => {
  const user = await requireAdmin(request, reply, [...publishRoles]); if (!user) return;
  const { id } = request.params as { id: string }; const body = request.body as { sourceName?: string; organization?: string; sourceUrl?: string; electionId?: string };
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const row of batch.rows) {
      ...
      const result = await tx.candidateResult.upsert({
        ...
        create: { electionId: r.electionId, constituencyVersionId: r.constituencyVersionId, candidateId: r.candidateId, partyId: r.partyId, votes: r.votes, voteShare: r.voteShare, position: r.position, isWinner: r.isWinner },
        update: { partyId: r.partyId, votes: r.votes, voteShare: r.voteShare, position: r.position, isWinner: r.isWinner }
      });
      await tx.resultSource.upsert({ where: { resultId_sourceId: { resultId: result.id, sourceId: source.id } }, create: { resultId: result.id, sourceId: source.id }, update: {} });
    }
