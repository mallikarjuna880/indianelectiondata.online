...
export async function listStates(query: Record<string, unknown>) {
  ...
  const [data, total] = await prisma.$transaction([
    prisma.state.findMany({ where, skip, take, orderBy: { name: 'asc' }, include: { _count: { select: { constituencies: true } } } }),
    prisma.state.count({ where })
  ]);
  return { data, pagination: pageMeta(page, pageSize, total) };
}

export async function listConstituencies(stateId: string | undefined, query: Record<string, unknown>) {
  const { skip, take, page, pageSize } = pagination(query);
  const q = String(query.q || '').trim();
  const where = { ...(stateId ? { stateId } : {}), ...(q ? { name: { contains: q, mode: 'insensitive' as const } } : {}) };
  const [data, total] = await prisma.$transaction([
    prisma.constituency.findMany({ where, skip, take, orderBy: { name: 'asc' }, include: { state: true } }),
...
export async function electionResults(electionId: string, query: Record<string, unknown>) {
  const { skip, take, page, pageSize } = pagination(query);
  const constituencyVersionId = String(query.constituencyVersionId || '');
  const partyId = String(query.partyId || '');
  const where = { electionId, ...(constituencyVersionId ? { constituencyVersionId } : {}), ...(partyId ? { partyId } : {}) };
  const [data, total] = await prisma.$transaction([
    prisma.candidateResult.findMany({ where, skip, take, orderBy: [{ constituencyVersionId: 'asc' }, { position: 'asc' }], include: { candidate: true, party: true, constituencyVersion: { include: { constituency: { include: { state: true } } } } } }),
...
export async function search(query: Record<string, unknown>) {
  const { skip, take, page, pageSize } = pagination(query);
  const q = String(query.q || '').trim();
  if (q.length < 2) return { data: [], pagination: pageMeta(page, pageSize, 0), message: 'q must contain at least 2 characters' };
...
export async function publishedElections(query: Record<string, unknown>) {
  const { skip, take, page, pageSize } = pagination(query);
  const type = String(query.electionType || '');
  const where = { sourceStatus: 'PUBLISHED' as const, ...(type ? { electionType: type as any } : {}) };
  const [data, total] = await prisma.$transaction([prisma.election.findMany({ where, skip, take, orderBy: [{ year: 'desc' }, { name: 'asc' }] }), prisma.election.count({ where })]);
  return { data, pagination: pageMeta(page, pageSize, total) };
}

export async function electionSummary(id: string) {
  const election = await prisma.election.findFirst({ where: { id, sourceStatus: 'PUBLISHED' }, include: { _count: { select: { results: true, versions: true } } } });
  if (!election) return null;
  const stats = await prisma.constituencyStatistic.aggregate({ where: { electionId: id }, _sum: { electors: true, votesPolled: true, validVotes: true, notaVotes: true }, _avg: { turnoutPercentage: true } });
  return { election, statistics: stats };
}
