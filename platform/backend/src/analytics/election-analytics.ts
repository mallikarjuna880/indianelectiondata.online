...
export async function getElectionSummary(electionId: string): Promise<ElectionSummary | null> {
  ...
  const turnout = electors > 0 ? round((votesPolled / electors) * 100) : turnoutValues.length ? round(turnoutValues.reduce((a, b) => a + b, 0) / turnoutValues.length) : null;
  return {
    electionId,
    seatsContested: seats.size,
    seatsWithResults: new Set(results.filter((r) => r.isWinner).map((r) => r.constituencyVersionId)).size,
    candidates: new Set(results.map((r) => r.candidateId)).size,
    parties: new Set(results.map((r) => r.partyId).filter((id): id is string => Boolean(id))).size,
...
export async function getElectionStatePerformance(electionId: string): Promise<StatePerformance[] | null> {
  ...
  const electionTotal = results.reduce((sum, r) => sum + r.votes, 0);
  ...
  for (const result of results) {
    const stateId = result.constituencyVersion.constituency.state.id;
    const set = seatSets.get(stateId) ?? new Set<string>();
    set.add(result.constituencyVersionId);
    seatSets.set(stateId, set);
  }
  for (const item of totals.values()) {
    item.seatsWithResults = seatSets.get(item.stateId)?.size ?? 0;
    item.voteShare = electionTotal ? round((item.votes / electionTotal) * 100) : 0;
  }
...
export async function getElectionMargins(electionId: string): Promise<ConstituencyMargin[] | null> {
  ...
  return [...grouped.values()].map((items) => {
    ...
    const runnerUp = winner ? items.find((r) => r.candidateId !== winner.candidateId) ?? null : null;
    return {
      ...
      winner: { candidateId: winner.candidateId, candidateName: winner.candidate.name, partyId: winner.party?.id ?? null, partyName: winner.party?.name ?? null, votes: winner.votes, voteShare: winner.voteShare == null ? null : Number(winner.voteShare) },
...
export type ElectionTurnout = {
  ...
  constituencies: Array<{ constituencyVersionId: string; electors: number | null; votesPolled: number | null; validVotes: number | null; notaVotes: number | null; turnoutPercentage: number | null }>;
};

export async function getElectionTurnout(electionId: string): Promise<ElectionTurnout | null> {
  const election = await prisma.election.findFirst({ where: { id: electionId, ...publishedElection } });
  if (!election) return null;
  const statistics = await prisma.constituencyStatistic.findMany({ where: { electionId, election: publishedElection }, orderBy: { constituencyVersionId: 'asc' }, select: { constituencyVersionId: true, electors: true, votesPolled: true, validVotes: true, notaVotes: true, turnoutPercentage: true } });
  const electors = statistics.reduce((sum, s) => sum + (s.electors ?? 0), 0);
  const votesPolled = statistics.reduce((sum, s) => sum + (s.votesPolled ?? 0), 0);
  const validVotes = statistics.reduce((sum, s) => sum + (s.validVotes ?? 0), 0);
  const notaVotes = statistics.reduce((sum, s) => sum + (s.notaVotes ?? 0), 0);
  return {
    electionId,
    electors,
    votesPolled,
    validVotes,
    notaVotes,
    turnoutPercentage: electors ? round((votesPolled / electors) * 100) : null,
    constituencies: statistics.map((s) => ({ constituencyVersionId: s.constituencyVersionId, electors: s.electors, votesPolled: s.votesPolled, validVotes: s.validVotes, notaVotes: s.notaVotes, turnoutPercentage: s.turnoutPercentage == null ? null : Number(s.turnoutPercentage) }))
  };
}
...
export async function getConstituencyAnalytics(constituencyId: string, electionId?: string) {
  ...
  const runnerUp = winner ? version.results.find((r) => r.candidateId !== winner.candidateId) : undefined;
  return {
