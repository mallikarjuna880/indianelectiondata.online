...
export function rankParties(data: Awaited<ReturnType<typeof getPartyComparison>>, metric: RankingMetric) {
  if (!data) return null;
...
export async function registerPartyAnalyticsApi(app: FastifyInstance) {
  ...
  app.get('/api/v1/analytics/elections/:id/party-rankings', async (request, reply) => {
    const { id } = request.params as { id: string }; const { metric = 'seats' } = request.query as { metric?: string };
    ...
    const data = rankParties(await getPartyComparison(id), metric as RankingMetric); if (!data) return reply.code(404).send({ error: 'ELECTION_NOT_FOUND' }); return { metric, data };
  });
  app.get('/api/v1/analytics/elections/:id/party-compare', async (request, reply) => {
    const { id } = request.params as { id: string }; const { partyIds = '', compareElectionId } = request.query as { partyIds?: string; compareElectionId?: string };
    const selected = partyIds.split(',').map((v) => v.trim()).filter(Boolean); if (selected.length > 10) return reply.code(400).send({ error: 'TOO_MANY_PARTIES', max: 10 });
    const current = await getPartyComparison(id); if (!current) return reply.code(404).send({ error: 'ELECTION_NOT_FOUND' });
    const currentRanked = rankParties(current, 'seats') ?? []; const currentRows = selected.length ? currentRanked.filter((p) => p.partyId && selected.includes(p.partyId)) : currentRanked;
    if (!compareElectionId) return { electionId: id, data: currentRows };
    const previous = await getPartyComparison(compareElectionId); if (!previous) return reply.code(404).send({ error: 'COMPARE_ELECTION_NOT_FOUND' }); const previousRanked = rankParties(previous, 'seats') ?? []; const previousMap = new Map(previousRanked.map((p) => [p.partyId, p]));
